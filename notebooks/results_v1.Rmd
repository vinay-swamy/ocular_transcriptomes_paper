...
```
We constructed transcriptomes for each tissue in the following manner: use Stringtie on each sample, then merge gtfs for each sample by subtissue, using a TPM cutoff of of at least 1 TPM in halfddd in each tissue. We then evaluated this using our salmon expression metrics. We found that transcript contruction is largely similar between subtissues, ie in the brain, which gives some measure of sanity to our results.
