...
|              Finally, we applied this optimized pipeline to a data set containing `r toString(NUM_EYE_SAMPLES)` human ocular tissue samples compiled from mining previously published, publicly available short-read RNA-seq data [@swamy_eye_2019]. We built transcriptomes for three major ocular tissues: cornea, retina, and RPE, using RNA-seq data from both adult and fetal tissues to create a high-quality pan-eye transcriptome. In addition to ocular samples, we used 877 samples from the GTEx data set to construct transcriptomes for tissues in `r toString(NUM_BODY_TISSUES)` other locations across the body. We refer to each distinct body location as a subtissue here after.
...
VSS and DMM designed overall study. VSS implemented all computational pipelines and analysis. TGF and AM conducted all bench experiments. VSS, DM, TGF, and RBH drafted and edited this manuscript. 
