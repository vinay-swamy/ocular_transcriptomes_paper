...
A historical criticsm of *de novo* transcriptome construction is the inability to evaluate the accuracy of construction methods. Long read sequencing technologies are a solution to this problem.  Historically, long read technologies have been too innaccurate to be reliably used, but the recently published PacBio sequel II system nearly was shown to be as accurate as short read sequencing. Long read sequencing captures entire full length transcripts, and provide a more reliable ground truth to evaluate short read based transcriptomes.
In this study we *de novo* constructed the transcriptome of the eye using a previously published data set containing over 800 ocular tissue samples compiled from mining publicly available sequencing data. [@swamy_eye_2019]. We first genreate pacbio long read RNA seq data from a stem cell derived retinal pigmented epithelium(RPE) cell line, and use it to design a calibrated and highly accurate de novo transcriptome construction pipeline. We use this pipeline build transcirptomes in three major ocular subtissues: The cornea, retina, and the retinal pigmented epithelium (RPE), using RNA-seq data from both adult and fetal tissues  to create a high-quality pan-eye transcriptome, revealing hundreds of novel gene isoforms as well as several novel genes. In addition to our ocular samples, we used a subset of the GTEx dataset to construct transcriptomes for 49 other locations across the body to facilitate comparisons in transcriptomes across the body. Here, we provide our *de novo* transcriptomes as a resource to other researchers through an R package and webapp.

# Methods 

![fig](/data/swamyvs/ocular_transcriptomes_paper/clean_data/plots/dntx_flowchart.png)
...
## Code availability.
|        To improve reproducibility, we wrote all code used to generate both the data and figures for this paper as Snakemake pipelines.[-@koster_snakemakescalable_2012] All code used for this projecgt is publicly available in the follwoing github repositories: https://github.com/vinay-swamy/ocular_transcriptomes_pipeline - main pipeline, 
...
|       Our transcriptomes removed on average of `r toString(NUM_AVG_TXOME_SIZE_DECREASE)` % of the a tissue's base transcriptome. We define base transcriptome for a tissues as any transcript in the gencode annotation with non zero TPM in at least one sample of a given tissue type. As this was a strikingly large reduction in transcriptome size, we wanted to ensure we were not unduly throwing away data. We quantified transcript expression of our samples using the alignment-free quantification tool Salmon, quantifiying each sample twice: once using the gencode V27 human transcript annotation, and once using its associated tissue specific transcriptome We found that despite the `r toString(NUM_AVG_TXOME_SIZE_DECREASE)` % reduction in number of transcripts between the base gencode and denovo transcriptomes, there is only an average decrease of `r toString(NUM_AVG_MAP_RATE_DIF)` % in mapping rate, indicating that majority of gene expression data is preserved.  
