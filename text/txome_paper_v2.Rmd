...
load('/data/swamyvs/ocular_transcriptomes_pipeline/ref/rdata/core_tight.Rdata')
...
This lead to XX% increase in accuracy, but still overall low accuracy. We also see a clear iniability for pacbio to detect transcripts below 2000 base pairs at this library size preparation. Within the stringtie build we see that many of the constructed transcripts are below this level, and therefore cannot be directly compared. 
...
isotype_per_tissue + ((novel_transcript_types + novel_exon_types) / exon_type_tx_plot ) +plot_annotation(tag_levels = 'A')
...
|        Novel isoforms occur due to a novel arrangement of known exons or contain a novel exon. A novel arrangment of known exons presents as the ommission of one or more exons, where as novel exons are either a modification of a known exon to generate a novel modified exon, or an addition of a previously unannotated  cryptic exon. For novel exon, we further annotated them with the putative biological process driving their inclusion: alternative splicing -> A3SS/A5SS , alternative promoter usage -> novel TES, or alternative polyadenylation -> Novel TES.
...
Next, we analyzed the novel isoforms within our ocular transcriptomes. We compared the overlap in constructed novel isoforms across ocular tissues and found that XXX % of novel isoforms are specific to a singular ocular subtissue (fig 3a), but there are significant transcripts for pan-eye and fetal-like tissues as well. For each novel isoform we then calculated percent isoform usage (PIU), or the fraction of total gene expression a transcript contributed to its parent gene. We found that on average for retina and RPE, novel isoforms contribute to 30 % of their parent gene’s expression. 

De novo transcriptomes allows for easy identification of the primary isoform expressed in a tissue, or any isoform that has piu >XXX(.95)
...
adult_tissues <- eye_tissues[grepl('Adult', eye_tissues)]
fetal_tissues <- eye_tissues[!grepl('Adult', eye_tissues)]
high_piu <- piu_df %>% 
  filter(piu >=.95) %>% inner_join( plot_df) %>% 
  mutate(tot = rowSums(.[,eye_tissues]), 
         adult = rowSums(.[,adult_tissues]), 
         fetal = rowSums(.[,fetal_tissues])) %>% 
  inner_join(gtf %>% filter(type == 'transcript') %>% select(transcript_id, gene_name))
T_spec_tx <- lapply(eye_tissues,function(x) high_piu %>% filter(.[,x],tot == 1) %>% pull(gene_name))
names(T_spec_tx) <- eye_tissues
pan_eye_tx <- filter(high_piu, tot == 6) %>% pull(gene_name)
fetal_tx <- filter(high_piu, fetal>1, adult == 0 ) %>% pull(gene_name)
save(pan_eye_tx, fetal_tx, T_spec_tx, file = 'testing/high_piu_tx.Rdata')
```
...
```{r}
load('clean_data/rdata/hmmer_results.Rdata')
tc2ms<- fread('../ocular_transcriptomes_pipeline/data/gtfs/all_tissues.convtab') %>% as_tibble 
det_mat <- apply(tc2ms[,-(1:4)],2, function(t) t!='') %>% cbind(tc2ms[,1:4], .)
eye_tissues <- c('Retina_Fetal.Tissue', 'Retina_Adult.Tissue', 'RPE_Fetal.Tissue', 'RPE_Adult.Tissue',
                 'Cornea_Adult.Tissue', 'Cornea_Fetal.Tissue', 'Lens_Stem.Cell.Line', 'ESC_Stem.Cell.Line')
body_tissues <- filter(sample_table, !subtissue%in% eye_tissues) %>% pull(subtissue) %>% unique
det_eye_body <- det_mat %>% mutate(det_eye  = rowSums(.[,eye_tissues]), det_body = rowSums(.[,body_tissues])) %>% 
  select(transcript_id, det_eye, det_body)

gtf_pc_seq_hits <-  filter(gtf, transcript_id %in% seq_hits_eye$query_name, transcript_type == 'protein_coding') 

seq_hits_pc_multiexonic <-  filter(gtf, transcript_id %in% seq_hits_eye$query_name, transcript_type == 'protein_coding', exon_number == 2) %>% pull(transcript_id) %>% 
  {filter(seq_hits_eye, query_name %in%.)} %>% rename(transcript_id = query_name) %>% inner_join(det_eye_body) %>% 
  filter(det_body == 0)
gtf_seqhits_rpe_only <- seq_hits_pc_multiexonic %>% filter(RPE_Fetal.Tissue) %>% {filter(gtf, transcript_id %in% .$transcript_id)} %>% 
  filter(type == 'transcript')
RPE_novel_seq_hits_bed <-  gtf_seqhits_rpe_only %>% mutate(score = 333) %>% select(seqid, start, end, transcript_id, score, strand) 

write_tsv(RPE_novel_seq_hits_bed, '../ocular_transcriptomes_pipeline/testing/RPE_specific_novel_loci.bed')


k <- rtracklayer::readGFF('../pacbio_testing/data/talon_results/RPE_Fetal.Tissue/RPE_Fetal.Tissue_talon_observedOnly.gtf')
...
#     theme(axis.text.x=element_text(angle=45, hjust = 1), panel.background = element_blank())
# 
# novel_orfs_per_tissue <- novel_orfs_per_tissue %>% 
#   gather(key = 'transcript_type', value = 'counts', -body_location, -color) %>% 
#     filter(transcript_type!='color',!body_location %in% c('Lens_Stem.Cell.Line', 'ESC_Stem.Cell.Line')) %>%    
#     mutate(counts=as.numeric(counts), body_location_pretty=gsub('_|\\.', ' ', body_location))
# orf <- ggplot(data = novel_orfs_per_tissue) +
#   geom_col(aes(x=body_location_pretty, y=counts, fill=body_location, alpha=transcript_type), position = 'dodge') +
#     scale_fill_manual(values = color_list) +
#     scale_alpha_discrete(range=c(.5,1)) +
#     ggtitle('Novel Isoforms Contructed Across the Body')+
...
# AC
# BC'
# isoforms /orf/loci  +plot_annotation(tag_levels = 'A')
...
#     df <- df %>% 
#       mutate(weight = cumsum(df$count) / sum(df$count),
#               X = (i-.25),
#              Xend  = (i+.25),
#             Y <- c(0,df$weight[1:nrow(df)-1]),
#             end <- df$weight )
#     return(df)
#   }
...
#          pt = 'mapping_rate')
# NUM_GENCODE_TX = 203835
# size_reduction_df <- tx_counts %>% mutate(percent_txome_size_decrease = (NUM_GENCODE_TX -filtered) / NUM_GENCODE_TX) %>% 
