...
|       Our transcriptomes removed  on average of 80% of the a tissue's base transcriptome. We define base transcriptome for a tissues as any transcript with non zero TPM in at least one sample of a given tissue type. As this was a strikingly large reduction in transcriptome size,   we quantified transcript expression of our samples using the alignment-free quantification tool Salmon,quantifiying each sample twice: once using the gencode V27 human transcript annotation, and once using its associated tissue specific transcriptome. We found that  despite the 80% reduction in number of transcirpts, there is only an average decrease of 6.4% in mapping rate, Indicating that majority of gene expression data is perserved, any that many of the transcripts present in the gencode anotation are found at non significant levels across multiple tissues. This is further supported by the fact that across all tissues, only 60% of the gencode reference was recaptured.
...
The primary expressed isoform of a gene is defined as <XXX>. The APPRIS project identified primary isoforms for humans, but as different tissues express differnt isoforms, this designatation may be inaccurate in certain tissues. For each gene, we determined a tissue
