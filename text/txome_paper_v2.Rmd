...
library(grid)
library(png)
sample_table <- read_tsv('/data/swamyvs/ocular_transcriptomes_pipeline/sampleTableFullV3.tsv') %>% filter(subtissue != 'synth')
...
BBBBBB
'
length_upset/tpm_exp_plot +plot_annotation(tag_levels = 'A')
...
isotype_per_tissue <- ggplot(data = novel_isoforms_per_tissue) +
    geom_col(aes(x=transcript_type, y=counts, fill = body_location), position = 'dodge') +
    scale_fill_manual(values = color_list) +
    scale_alpha_discrete(range=c(.5,1)) +
    ggtitle('Novel Isoforms Contructed Across the Body')+
    ylab('number of novel transcripts')+
...
AC
'
isotype_per_tissue + loci + exon_type_tx_plot +plot_annotation(tag_levels = 'A') +plot_layout(design = layout)
