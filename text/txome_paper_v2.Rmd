...
We apply this pipeline to construct a pan eye transcriptome using a previously published data set containing `r toString(NUM_EYE_SAMPLES)` ocular tissue samples compiled from mining publicly available sequencing data. [@swamy_eye_2019]. We use this pipeline build transcirptomes in three major ocular subtissues: The cornea, retina, and the retinal pigmented epithelium (RPE), using RNA-seq data from both adult and fetal tissues  to create a high-quality pan-eye transcriptome. In addition to our ocular samples, we used a subset of the GTEx dataset to construct transcriptomes for 49 other locations across the body to facilitate comparisons in transcriptomes across the body.
