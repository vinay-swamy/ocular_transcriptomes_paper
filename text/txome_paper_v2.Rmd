...
NUM_STUDIES = 32
NUM_REF_TX_BODY = 287945
NUM_TOTAL_TX_BODY = NUM_NOVEL_TX_BODY + NUM_REF_TX_BODY
```
...
|     When examining the transcript lengths of each build we see that the two methods show very different transcript length distributions, with the short read build comprised mostly of smaller transcripts.(Fig1A) As the PacBio data was generted using two librarys for XXXBP and YYYBP, this is expected we expect to see enrichment for longer transcripts in the pacbio. To get a better sense of accuracy, we group transcripts by length, and compare accruacy between length group. This lead to XX% increase in accuracy, but still overall low accuracy. As there is clear inability within this  pacbio data to accurate determine transcripts below 2000BP, we cannot directly compare the accuracy for these smaller transcripts between the two builds.
|     We experimented with various methods to remove spurious transcirpts to improve construction accuracy, and found that removing transcripts that were expressed below an average TPM threshold was the most effective(Fig1b)  We note that this achieves significanlty higher accuracy than using the default filtering scheme provided in stringtie, merge. In our downstream pipeline we use an average TPM of 1 as our filtering threshold, as this achiaeves XXX% accuracy in transcripts longer than 2000BP, as we reason that transcripts below 2000BP follow a simialr TPM distribution as those above.(this should probably be a supplimental figure )
...
Novel transcripts are split into two categories: novel isoforms, which are novel variations of known genes, and novel loci, which are previously unreported, entirely novel regions of transcribed sequence. Novel isoforms are further classified by the novelty of its encoded protien, ie novel ORF, a novel isoform with a known ORF, and isofroms with no orf as noncoding isoforms. The number of distinct ORFs is significanlty less than the number of transcripts, with XXX previously annotated ORFs and YYY novel ORFs.
...
map_rate_diffs <- all_sample_mapping_rate_difference %>%
  filter(!body_location %in% c('Lens_Stem.Cell.Line', 'ESC_Stem.Cell.Line')) %>%
  mutate(body_location_pretty=gsub('_|\\.', ' ', body_location),
         body_location=case_when(body_location == 'Body' ~ 'Body(avg)',
...
size_reduction_df  <-  inner_join(gencode_tx_size, tx_counts) %>%
  ...
  mutate(pt = 'txome rate')


ggplot(map_rate_diffs) +
  geom_boxplot(aes(x = pt, y = mapping_rate_diff, fill = body_location))+
  geom_col( data = size_reduction_df, aes(x = pt, y=percent_txome_size_decrease, fill = body_location)) +
  scale_fill_manual(values = color_list) +
  theme(axis.text.x=element_text(angle=45, hjust = 1), panel.background = element_blank()) +
  facet_wrap(~body_location, ncol = 2) +
...
|       Our transcriptomes removed on average of 80% of the a tissue's base transcriptome. We define base transcriptome for a tissues as any transcript with non zero TPM in at least one sample of a given tissue type. As this was a strikingly large reduction in transcriptome size, we wanted to ensure we were not unduly throwing away data.  We quantified transcript expression of our samples using the alignment-free quantification tool Salmon,quantifiying each sample twice: once using the gencode V27 human transcript annotation, and once using its associated tissue specific transcriptome We found that despite the 80% reduction in number of transcripts between the base gencode and denovo transcriptomes, there is only an average decrease of 6.4% in mapping rate, indicating that majority of gene expression data is perserved.  Across all tissues, only 60% of the gencode reference was recaptured indicating that much of the existing refernce may be redundant.

## De novo transcriptomes identify of the tissue specific primary isoform expressed in a tissue

|     It is useful researchers to know which isoform of a gene is the most relecant to a particular biological system. The APPRIS project identified principal isoforms for humans. based on predicted function and evolutionary conservation, using multiple computational tools. Principal isoforms are considered the most biologically functionaly isoforms of transcripts  We instead propose a simpler but higher resolution tissue specific primary isoforms, defined as the highest expressed isoforms of a given gene in a given tissue. As primary isoforms are based on biological evidence via RNA-seq, we reason that it is more accurate ontology than APPRIS principal isoform.
...
primary_isoforms_per_tissue <- primary_isoforms_per_tissue %>% 
  ...
  mutate(body_location_pretty=gsub('_|\\.', ' ', body_location),
         body_location=case_when(body_location == 'Body' ~ 'Body(avg)',
                                 body_location == 'Brain'~ 'Brain(avg)' ,
                                 TRUE ~ body_location)) %>% 
  group_by(body_location, primary_isoform_origin) %>%
  summarise(count = mean(count)) %>% 
  arrange()
ggplot(data = primary_isoforms_per_tissue, aes(x = body_location, y=count, fill = primary_isoform_origin))+
  geom_bar(position = 'fill', stat = 'identity')+
...
|    We found that XXX% of the APPRIS principal isoforms are also identified as the primary isoform in at least one tissue type, and that on average YYY% of transcripts per tissue. Within each tissue type, XXX% of transcripts were a previously annotated gencode transcript. Only ZZZ% of principal isoforms are novel isoforms. 
...
us <- ComplexUpset::upset(data = plot_df, 
                          ...
                          themes=upset_modify_themes(
                            list(
                              ...
                              "Intersection size"= theme(text = element_text(size=FIG_FONT_SIZE, angle = -45) ),
...
(us)/(piu )+ plot_annotation(tag_levels  = 'A') & theme(plot.tag = element_text(size= FIG_FONT_SIZE), text = element_text(size = FIG_FONT_SIZE)) 
...
n=15
vep_exp_all_eye <- vep_impact_matrix %>% 
  filter(rowSums(.[,eye_tissues] == 3) ==6, one_count>=n)
vep_no_eye <- vep_impact_matrix %>% 
  filter(rowSums(.[,eye_tissues] == 1) <=2,  three_count >=n/2, two_count >=3) 
...
::: {custom-style="CustomCaption"}
Figure 7. Tissue specific variant prioritization dot plot. Size corresponds to average TPM and collor to impact. A full mapping of variant descriptors to impact can be found in supplemental data
:::

Using our tissue specific transcriptome we reclassifed XXX variants, and found that the highest priority of XXX decreased, and YYYY increased. We see a large decrease in predicted effect as the transcript which previously lead to an effect is not expressed in a respective tissue. In addition, we find ZZZ variants that map to novel regions, and find CCC tissues specfic variants. 
...
|       Motivated by the lack of a comprehensive pan eye transcriptome and so we created the first comprehensive set of ocular transcriptomes.

By using long read RNA-sequencing data to calibrate our short-read construction pipeline, we are confident we created real, biologically relevant transcriptomes.  

By calibrating our short read constrution pipeline with long read data, our transcriptomes are more accurate than previous work. 
...
We used a large dataset compiled from published RNA-seq data to build these transcriptomes, an approach which has several key advantages. First, our large sample size allows us to combat the noisy nature of RNA-seq data. More importantly as our samples originate from multiple studies with multiple types of sample preparation, we can be more confident that our transcriptomes accurately reflect the biology of its originating subtissue and are not an artifact due to preparation of the samples. 
...
|      In our ocular transcriptomes, we see that novel isoforms are largely subtissue-specific. This matches previously reported findings about the tissue specificity of rare exons and first and last exons. We also find that on average novel isoforms represent about 15% of their parent.While this seems fairly low, it is difficult to say these are non-functional. Its is entirely possible that these isoforms are from low population cell types. This especially makes sense in the retina, bipolar/amacrine cell types comprise ~15% of the total cell population.  
...
|       We want to make our transcriptomes easily accessible to other researchers, so we designed a webapp to visualize our transcriptomes and access tissue-specific annotation files. The  work we have is more of creating a an ideal starting point for others to explroe in and many studies simply publish data and dont do anything with it, and so we want to make it easy for others to use, both with our app and containerized pipeline. 
      
      
As with any purely computational study, results must be taken with a grain of salt. 
