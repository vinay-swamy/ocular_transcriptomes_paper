...
development[-@dykes_hic2_2018], cell differentiation[-@trapnell_transcript_2010],
and cell migration[-@mitra_splicing_2020]. Alternative usage of isoforms has also
been implicated in multiple diseases including cancer[-@vitting-seerup_landscape_2017],
cardiovascular diseases[-@noauthor_titin_nodate], Alzheimer's
Disease[-@mills_rna-seq_2013] and diabetic
retinopathy[-@perrin_diabetic_2005].(didnt read these papers so need to double
check them)
...
coverage.[-@nagalakshmi_transcriptional_2008] More recently, several groups have developed
...
[-@haas_novo_2013],(2),[-@pertea_stringtie_2015].\
...
body. (10),[-@gtex_consortium_genetic_2017],[-@pertea_chess_2018] However, as the GTEx
...
sequences long reads as accurately as short read based seqencing [-@wenger_accurate_2019].
...
a previously published data set containing  `r toString(NUM_EYE_SAMPLES)` ocular tissue samples compiled from mining publicly available sequencing data [@swamy_eye_2019].We use this
...
{== fig ==}{>> (TYPOS in figure, also we use illumina short-read data as input, cruddy quality of right top corner) <<}
...
the culturing protocol in [-@blenkinsop_human_2015]. RNA was isolated from mature RPE 40 days
...
read using the Pacbio SMRT link software [VERSION]. We next adapted the ENCODE Long Read analysis pipeline:  Transcripts were aligned to the human genome using the minimap2[-@li_minimap2_2018]. Aligned long
reads were processed with TranscriptClean[-@wyman_transcriptclean_2019] to remove
errors due to sequencing. The long read transcriptome was generated with TALON
[-@wyman_technology-agnostic_2020]
...
sort.[-@frankish_gencode_2019],[-@dobin_star_2013],[-@li_sequence_2009].
...
transcriptome using gffcompare[-@pertea_gff_2020]. 
...
`r toString(NUM_BODY_TISSUES + NUM_EYE_TISSUES)` distinct subtissue regions of the body, downloaded and performed quality control the pertinent sequencing data from the sequence read archive (SRA) using methods from our previous work[-@swamy_eye_2019]. We constructed a transcriptome for each sample, and
merged samples together to create `r toString(NUM_BODY_TISSUES + NUM_EYE_TISSUES)` tissue specific base
...
quantification tool Salmon.[-@patro_salmon_2017] For each sample, we
...
Analysis of novel exons was done using a custom Rscript {== "annotate_and_make_tissue_gtfs.R" ==}{>> This step is part of a larger script might consider separating into its own script. <<}. To predict the clinical significance of clinvar variants of uncertain
...
base using blastp[-@altschul_basic_1990]. Blastp results were integrated with
...
data and figures for this paper as Snakemake pipelines.[-@koster_snakemakescalable_2012]
...
programming language.[-@r_core_team_r_2019] A full list of packages and
...
`r toString(NUM_TOTAL_LR_TX)` distinct transcripts, and in our short read
transcriptome `r toString(NUM_TOTAL_SR_TX)` distinct transcripts

```{r longread_results, fig.height=7, fig.width=8}
...
`r toString(NUM_BASE_TXOME_CONST_ACC)`. When examining the transcript lengths of each build we
...
`r toString(NUM_TXOME_CONST_ACC_2000_ABOVE)`, and `r toString(NUM_TXOME_CONST_ACC_2000_BELOW)` for transcripts below
2000 bp\
...
recomended protocol.[-@pertea_transcript-level_2016]This acheived
`r toString(NUM_TXOME_MERGE_ACC_2000_ABOVE)` for transcripts longer than 2000bp, and
`r toString(NUM_TXOME_MERGE_ACC_2000_BELOW)` for transcripts shorter than 2000bp. As this accuracy
...
read samples. This achieved a build accuracy of `r toString(NUM_FILT_TXOME_ACC_2000_ABOVE)` and
retatining `r toString(NUM_TXOME_FILTER_SIZE)` transcripts. In our downstream pipeline we
...
      We built transcriptomes from `r toString(NUM_EYE_SAMPLES)` published, publicly
...
tissues mined from `r toString(NUM_STUDIES)` different studies (Table 1). Our
...
with `r toString(NUM_BODY_SAMPLES)` samples across `r toString(NUM_BODY_TISSUES)` body
...
the number of transcripts, with `r toString(NUM_REF_ORF)` previously annotated
ORFs and `r toString(NUM_NOVEL_ORF)` novel ORFs across all tissues. Across all
tissues there is an average of `r toString(NUM_AVG_NOVEL_ISOFORM_PER_TISSUE)` novel isoforms and
`r toString(NUM_AVG_NOVEL_ORF_PER_TISSUE)` novel ORFs.
...
      Our transcriptomes removed on average of `r toString(NUM_AVG_TXOME_SIZE_DECREASE*100)` % of
...
the `r toString(NUM_AVG_TXOME_SIZE_DECREASE*100)` % reduction in number of transcripts between the
base gencode and denovo transcriptomes, there is only an average decrease of
`r toString(NUM_AVG_MAP_RATE_DIF*100)` % in mapping rate, indicating that majority of gene
...
   We found that `r toString(NUM_APPRIS_RECOVERY_RATE)` of the APPRIS principal isoforms are
also identified as the primary isoform in at least one tissue type. Within each
tissue type, on average `r toString(NUM_AVG_APPRIS)` % of are an appris primary
isoform, `r toString(NUM_AVG_NONAPPRIS_GENCODE)` of transcripts were a previously annotated
gencode transcript, `r toString(NUM_AVG_NOVEL)` \[THIS IS WRONG\] % of principal
...
`r toString(NUM_TOTAL_VUS)` total variants examined,the highest priority of
`r toString(NUM_IMPACT_DECREASE)` decreased in at least one tissue, and
`r toString(NUM_IMPACT_INCREASE)` increased in at least one tissue. We see a larger
...
`r toString(NUM_TXOME_CONST_ACC_2000_ABOVE)`. This was surprisingly low. It is difficult to say
...
Though we initially reconstructed `r toString(round(NUM_INITIAL_ANOO_TX/NUM_TOTAL_GENCODE_TX*100), digits = 3)` % of the gencode
trqnscriptome, but after filtering we were left with `r toString(round(NUM_REF_TX_BODY/NUM_TOTAL_GENCODE_TX*100), digits = 3)`
...
and lack biological significance.[-@xu_evidence_2019]. Previous research
...
The recent GnomAD[-@karczewski_mutational_2020] project showed that taking transcript
expression in to account can improve variant
prioritization.[-@noauthor_transcript_nodate] Our tissues specfic variant prioitization
